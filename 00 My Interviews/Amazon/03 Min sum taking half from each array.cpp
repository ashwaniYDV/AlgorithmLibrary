...
Find min total sum of elements in a1 and a2 by selecting one element for each index 
...
/*
One solution would be choosing all possible n/2 elements in a1 and a2 and computing their results. 
...
In a different way: 
we combine i-th indexes and compute a penalty = a1[i] - a2[i]:
...
/*
Amazon has hired n new employees (n is even). 
You have to send half of these employees to Bangalore and other half to Hyderabad. 
The cost for each employee for each city is given in a form of array. 
Find min cost.
...
int fun(int i, int cntOfBLR, vector < vector < int > >& cost) {
    if(i == n) return 0;

    int x = 1e9, y = 1e9;
    
    if(cntOfBLR < n/2) {
        x = cost[i][0] + fun(i + 1, cntOfBLR + 1, cost);
    }
    
    int cntOfHyd = i - cntOfBLR;
    if(cntOfHyd < n/2) {
        y = cost[i][1] + fun(i + 1, cntOfBLR, cost);
    }
