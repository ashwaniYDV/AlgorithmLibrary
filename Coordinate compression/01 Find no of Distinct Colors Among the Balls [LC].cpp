...
// Without corrdinate compression also we can do like this
class Solution {
public:
    vector<int> queryResults(int limit, vector<vector<int>>& queries) {
        unordered_map<int, int> colorToBallCnt;
        ...
        for(auto& it: queries) {
            ...
            if(ballColor.count(x)) {
                colorToBallCnt[ballColor[x]]--;
                if(colorToBallCnt[ballColor[x]] == 0) colorToBallCnt.erase(ballColor[x]);
            }
            colorToBallCnt[y]++;
            ballColor[x] = y;

            res.push_back(colorToBallCnt.size());
        }
...
// With corrdinate compression
class Solution {
public:
    void coordinateCompression(int& limit, vector<vector<int>>& queries) {
        set<int> st;
    ...
    vector<int> queryResults(int limit, vector<vector<int>>& queries) {
        coordinateCompression(limit, queries);
