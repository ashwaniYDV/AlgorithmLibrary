...
/*
You are given n​​​​​​ tasks represented by a 2D integer array tasks, 
...
public:
    vector<int> getOrder(vector<vector<int>>& tasks) {
        ...
        // sort on basis of (enqueueTime, processingTime, idx)
        ...
        while(!pq.empty() || i < n) {
            if (pq.empty()) {
                ...
                curTime = max(curTime, (long long)tasks[i][0]);
                i++;
            }
            ...
            auto it = pq.top(); 
            pq.pop();
            int idx = it[1], processingTime = it[0];
